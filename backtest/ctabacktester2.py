...
def run_StockIndexBacktester():
    sib = StockIndexBacktester(startdate=datetime(2015, 1, 1), enddate=datetime(2023, 1, 1))
    # sib.run_backtesting_s('IF000')
    # sib.symbol_li = ['IF']
    # sib.run_symbol_backtesting()
    sib.vol_adj_symbol_dynamicbacktesting(symbol='IC')
...
def run_backtest_statistic():
    bs = BacktesterStatistics()
    pa = './datas/backtest_res/vol_dow_dynamic_params/df_res.csv'
    sp = './datas/backtest_res/vol_dow_dynamic_params'
    df = pd.read_csv(pa)[['datetime', 'pnl']]
    df_compound = bs.change_pnl_compound_interest(df, sp)
    df_train, df_test = bs.train_test_pnl_sep(df_compound, sp=sp)
    bs.get_yearly_statistic(df_train, f'{sp}/df_train_year.csv', dan=0)
    bs.get_yearly_statistic(df_test, f'{sp}/df_test_year.csv', dan=0)
