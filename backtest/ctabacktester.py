...
from backtest.strategies.maatr_strategy import MAATRStrategy
...
class MainconBackTester(BackTester):
    ...
    def symbol_backtesting(self, symbol, startdate=None, enddate=None, save_pa='', params={}, is_plot=1):   # save_pa='boll'
        '''单品种全合约回测'''
        if startdate==None:
        ...
        if len(df_res) > 50 and len(save_pa):
    ...
    def symbol_backtesting_dynamic_adj(self, startdate=None, enddate=None, load_pa='', save_pa='boll', params={}, symbol=None):
        '''改写回测传参顺序和pnl拼接,用于滚动回测'''
        df_r = self.symbol_backtesting(symbol, startdate=startdate, enddate=enddate, save_pa=save_pa, params=params)
        lp = f'{load_pa}/{symbol}_df_all_time.csv'
        if os.path.exists(lp):
            df_load = pd.read_csv(lp)
            df_concat = pd.concat([df_load, df_r], ignore_index=True)
            df_concat.drop_duplicates(subset=['datetime'], keep='first', inplace=True)
            df_concat.to_csv(lp, index=False)
        else:
            df_r.to_csv(lp, index=False)
            df_concat = df_r
        st = enddate - timedelta(days=400)
        df_concat['dt'] = pd.to_datetime(df_concat['datetime'])
        df_concat = df_concat[df_concat['dt'] > st]
        df_concat.reset_index(drop=True, inplace=True)
        df_concat['pnl'] = df_concat['pct_change'].cumsum()+1
        # print(symbol, startdate, enddate, st)
        if len(df_concat) > 50: 
            save_folder = makedir(f'{save_pa}/{symbol}') if self.res_pa in save_pa else makedir(f'{self.res_pa}/{save_pa}/{symbol}')
            df_concat.to_csv(f'{save_folder}/{symbol}_df_res.csv', index=False)
        return df_concat

    def symbol_backtesting_all(self, startdate=None, enddate=None, load_pa='', save_pa='boll', params={}, max_worker=4):
        '''全品种回测'''
        ...
        func_bt = partial(self.symbol_backtesting_dynamic_adj, startdate, enddate, load_pa, save_pa, params)
...
class DynamicBacktester(MainconBackTester):
    ...
    def vol_adj_dynamicbacktesting(self, startdate=None, enddate=None, sp='vol_boll_dynamic', vol_params=0.1, params0={'rate': 0.0002}):
        '''含有波动率调整的动态回测'''
        res_pa = makedir(f'{self.res_pa}/{sp}_params')
        load_pa = makedir(f'{self.res_pa}/{sp}/all_time_res')
        vol_p = f'{res_pa}/df_vol.csv'
        # vol_p = remove_file(f'{res_pa}/df_vol.csv')
        if startdate is None: startdate, enddate = self.startdate, self.enddate
        df_t = get_df_date(startdate, enddate)
        ...
        for i in range(len(df_t)):
            ...
            if first: first = 0
            else: startdate = enddate - timedelta(days=60)
            params = {'vol_pa': vol_p}
            params.update(params0)
            save_pa = makedir(f'{self.res_pa}/{sp}/{folder}')
            while symbol_n > len(os.listdir(save_pa)):
                self.symbol_backtesting_all(startdate, enddate, load_pa, save_pa, params, max_worker=4)
...
class IndexBacktester(DowBacktester):
    def __init__(self, startdate=datetime(2010, 1, 1), enddate=datetime(2023, 1, 1), strategy=IndexStrategy):
        super().__init__(startdate, enddate, strategy)
        self.capital = 10_000_000
    ...
    def run_parent(self):
        ...
        for i in range(10, 15):
            params0 = {'rate': 0.0002, 'atr_n': 21, 'win_n': 1, "index_ind": i}
            func_mp = partial(self.vol_adj_dynamicbacktesting, None, None, 
                            f'vol_myindex{i}_dynamic', 0.15, params0)
            child_process = multiprocessing.Process(target=func_mp)
            child_process.start()
            while True:
                if not child_process.is_alive():
...
def run_MainconBackTester(symbol):
    ...
    mbt.symbol_backtesting(symbol, save_pa='maatr')        # 1 step
...
def multi_p(sp='myma'):
    mbt = MainconBackTester()
    # db = DowBacktester
    save_pa = makedir(f'./datas/backtest_res/{sp}')
    sy_li = mbt.symbol_li
    ...
    while symbol_n > len(os.listdir(save_pa)):
        ...
        m_multiprocess(run_IndexBacktester, sy_li)
        if symbol_n == len(os.listdir(save_pa)): 
            break
...
def run_DowBacktester(symbol='I'):
    ...
    db.vol_adj_dynamicbacktesting(sp='vol_dow_dynamic', vol_params=0.15, params0={'rate': 0.00013})
    # db.caculate_level(pa='vol_dow_dynamic')
    print(symbol)
    # db.run_backtesting(contract='I000', pa='dow_signal')

def run_IndexBacktester(symbol='RB'):
    ib = IndexBacktester(startdate=datetime(2010, 1, 1), enddate=datetime(2023, 1, 1), strategy=MAATRStrategy)
    # ib = IndexBacktester(startdate=datetime(2010, 1, 1), enddate=datetime(2023, 1, 1), strategy=IndexStrategy)
    index_ind = 10
    ib.run_backtesting_s(contract=symbol+'000', pa='maatr')
    # params = {'rate': 0.0002, 'atr_n': 21, 'win_n': 1, "index_ind": index_ind}
    # params = {'rate': 0.0002, 'atr_n': 21}
    # ib.run_symbol_backtesting(symbol, sp=f'index{index_ind}', params=params, strategy=IndexStrategy)
    # db.symbol_backtesting_total(pa='myindex3') 
    # ib.vol_adj_dynamicbacktesting(sp=f'vol_myindex{index_ind}_dynamic', vol_params=0.15, params0={'rate': 0.0002})
    # ib.run_parent()
